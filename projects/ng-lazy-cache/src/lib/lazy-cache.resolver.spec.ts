...
import { LazyCache, GLOBAL_TTL_TOKEN, clearCache } from './lazy-cache.resolver';
...
describe('LazyCache Resolver with GLOBAL_TTL_TOKEN', () => {
  ...
  it('should return cached data on second call (using global TTL)', (done) => {
    ...
    const resolver = TestBed.runInInjectionContext(() =>
      LazyCache<string>(fetchFn, { key: 'test-key' })
    );
    const route: any = {};
    const state: any = {};
  ...
  it('should respect route-level TTL over global TTL', fakeAsync(() => {
    ...
    const resolver = TestBed.runInInjectionContext(() =>
      LazyCache<string>(fetchFn, { key: 'ttl-key', ttl: 1000 }) // 1s TTL
    );
    const route: any = {};
    const state: any = {};
  ...
  it('should invalidate cache after TTL expires', fakeAsync(() => {
    ...
    const resolver = TestBed.runInInjectionContext(() =>
      LazyCache<string>(fetchFn, { key: 'expire-key', ttl: 200 }) // 0.2s TTL
    );
    const route: any = {};
    const state: any = {};
  ...
  it('should handle fetch errors with fallback', (done) => {
    ...
    const resolver = TestBed.runInInjectionContext(() =>
      LazyCache<string>(fetchFn, { key: 'error-key' })
    );
    const route: any = {};
    const state: any = {};
